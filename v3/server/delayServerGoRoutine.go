...
func (s *DelayServer) GetDelayMsgGoroutine() {
	...
	//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_delay_message").Info("start")
	...
	for true {
		...
		if err != nil {
			if !ierrors.IsEqual(err, ierrors.ErrTypeEmptyQueue) {
				//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_delay_message").Error("get msg error, ", err)
				s.logger.ErrorWithField(fmt.Sprint("goroutine get_delay_message get msg error, ", err), "server", s.delayGroupName)
			}
			continue
		}

		//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_delay_message").Debug("get delay msg, ", msg)
	...
	//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_delay_message").Info("stop")
...
func (s *DelayServer) GetDelayMsgGoroutine_UpdateQueue(msg message.Message) {
	...
	if popMsg != nil {
		//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_delay_message").Debug("pop msg, ", *popMsg)
		...
		if err != nil {
			//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_delay_message").Error("Send msg error: ", err, " [msg=", *popMsg, "]")
...
func (s *DelayServer) GetReadyMsgGoroutine() {
	//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_ready_message").Info("start")
	...
	for true {
		...
		//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_ready_message").Debug("get ready msg: ", msg)
		...
		if err != nil {
			...
			if err != nil {
				//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_ready_message").Error("LSend msg error: ", err, " [msg=", msg, "]")
	...
	//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "get_ready_message").Info("stop")
...
func (s *DelayServer) SendReadyMsgGoroutine() {
	//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "send_ready_message").Info("start")
	s.logger.InfoWithField("goroutine send_ready_message start", "server", s.delayGroupName)

	for msg := range s.readyMsgChan {

		//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "send_ready_message").Info("send ready msg: ", msg)
		...
		if err != nil {
			err = s.broker.LSend(s.GetQueueName(s.delayGroupName), msg)
			if err != nil {
				//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "send_ready_message").Error("LSend msg error: ", err, " [msg=", msg, "]")
				s.logger.ErrorWithField(fmt.Sprint("goroutine send_ready_message LSend msg error: ", err, " [msg=", msg, "]"), "server", s.delayGroupName)
			}
		}
	}
	//log.TaskLog.WithField("server", s.delayGroupName).WithField("goroutine", "send_ready_message").Info("stop")
